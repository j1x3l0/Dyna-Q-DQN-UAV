...
BCAD（Bellman-Consistent Adaptive Dyna）使用模型 Bellman 目标与真实 Bellman 目标的归一化差异评估规划可信度，不再仅依据状态预测误差启用规划。默认配置保留 25,600 个真实时隙预热，随后用 8,000 步渐进提高规划概率；最大规划概率为 0.25，模型规划更新学习率为真实 DQN 更新的 10%，第一版关闭反事实动作。五种子筛查脚本为 `scripts/run_bcad_multiseed.py`。
