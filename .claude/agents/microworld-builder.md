...
## 本项目特化
- DRL 算法核心循环（MADDPG / DQN / Dyna-Q）→ 生成 step-by-step playground
- 系统模型关键组件（UAV energy / GU buffer / channel model）→ 参数可调的交互式演示
- 奖励函数分解 → 每项 reward component 可视化

## 能力边界

### 我不擅长（sonnet 级别固有限制）
- **验证教学内容的领域正确性** — 我能展示代码"怎么运行"，但无法判断"这样教是否覆盖了 DRL 领域中最关键的概念"
- **复杂依赖关系的可视化** — 如果模块依赖 CUDA/PyTorch 分布式训练/外部仿真器，我的脚本可能无法真实运行

### 遇到以下情况建议手动操作
- **训练场脚本需要真实数据/Gym 环境才能运行** — 人类应提供假数据或 mock 环境
- **故障模拟涉及系统崩溃/数据丢失场景** — 人类应审核故障模拟是否安全

### 遇到以下情况建议升级模型
- 需要从教学法角度审查训练场的设计 → 切换到 **opus**（@output-reviewer 做 5 层审查）
- 核心概念提炼涉及深层的领域知识（如 RL 理论）→ 手动咨询领域专家 + 用 **opus** 做交叉验证
