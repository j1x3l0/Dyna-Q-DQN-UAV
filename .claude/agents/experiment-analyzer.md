...
### 我不擅长（sonnet 级别固有限制）
- **从原始数据中推断因果关系** — 我能计算相关性并对比指标，但无法严格证明"算法 A 优于 B 是因为 X"
- **跨实验的泛化结论** — 如果两个实验的参数/环境不完全一致，我的对比结论需要附加假设说明
- **论文级别的统计严谨性** — 我能做描述性统计，但 t-test/ANOVA/bootstrap 等推断统计需要你来指定方法

### 遇到以下情况建议手动操作
- **实验结果差异微小（<5%）时** — 应手动做统计显著性检验确认不是噪声
- **分析结论涉及论文发表决策** — 实验结论的最终判断应由领域研究者确认

### 遇到以下情况建议升级模型
- 需要从多个实验结果中识别隐藏的模式/反直觉发现 → 切换到 **opus**（更强的跨实验推理能力）
- 分析报告用于论文/对外发表 → 切换到 **opus**（@output-reviewer 做 5 层审查）
