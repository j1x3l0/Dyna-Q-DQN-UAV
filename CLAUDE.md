...
Hierarchical-DynaQ (分层+模型)
  │  已验证：比 MADDPG 提升 63-76%  ← 历史结果（早期负奖励、2 seeds），已被阶段 B 取代
  │  当前主张：加快收敛 / 减少真实交互（样本效率），最终性能非核心
  │  未验证：模型预测误差对长期训练的影响；Dyna-K 已由阶段 D 正式固定为 K=1
  ▼
...
> ⚠️ **历史结果（2026-07 前，`ee_ratio` 负奖励口径、2 seeds）**：下表仅作历史记录，已由
> 阶段 B `paper_xi` 5-seed 证据取代。正式结论：DynaQ 与 NoDyna 最终性能基本持平
> （512.75 vs 515.02），Dyna 呈 1.59× 收敛加速的**描述性**趋势（不显著）；核心主张为
> **收敛加速 / 减少真实交互（样本效率）**，而非最终性能提升。详见
> `reports/阶段B五种子显著性检验报告_20260729.md`。
...
**2026-08-04 状态更新**：Dyna-K 消融完成并固定 **K=1**（K=5 无稳定收益、墙钟 +105.7%）；
warm-up 冻结 **128**（单 seed 工程选择）；阶段 E 扩展基线修复 + 参数门控完成，MATD3/CoP
均选 conservative（lr=1e-4）；阶段 E 正式 5-seed 训练就绪待启动。证据对照见
`reports/当前进度审查报告_20260803.md`。
